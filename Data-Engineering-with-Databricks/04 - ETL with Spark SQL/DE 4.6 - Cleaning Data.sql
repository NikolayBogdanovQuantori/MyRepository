...
SELECT 
  ...
  count(*) AS total_rows, 
  count(DISTINCT(*)) AS unique_non_null_rows -- задать вопрос
FROM users_dirty

-- COMMAND ----------

select distinct email from users_dirty order by email nulls first;
-- 139 строк

-- COMMAND ----------

select count(distinct email) from users_dirty;
-- 138 строк, так как строка с null проигнорирована

-- COMMAND ----------

select distinct user_id, email from users_dirty order by user_id nulls first, email nulls first;
-- 984 строки

-- COMMAND ----------

select count(distinct (user_id, email)) from users_dirty
-- 984 строки
...
SELECT * FROM users_deduped order by user_id, user_first_touch_timestamp
