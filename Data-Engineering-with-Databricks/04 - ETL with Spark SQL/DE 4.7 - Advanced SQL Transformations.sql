...
SELECT value:device, value:geo:city, value:items
FROM events_strings
...
FROM events
where user_id = "UA000000102359929"

-- COMMAND ----------

SELECT 
  collect_set(event_name), -- уникальные event_name по всему датасету
  collect_set(items.item_id), -- массив массивов, каждый из которых, это значение ключа items.item_id, при этом массивы внутри "внешнего" массива уникальны
  flatten(collect_set(items.item_id)), -- содержимое предыдущего столбца, конкатенированное в один массив
  array_distinct(flatten(collect_set(items.item_id))) -- содержимое предыдущего столбца без дубликатов, которые появляются в результате [A, B] + [B, C] = [A, B, B, C] 
FROM events
--WHERE user_id = "UA000000102359929";
...
SELECT order_id, king_items, size(king_items)
FROM (
  SELECT
    order_id,
    FILTER (items, i -> i.item_id LIKE "%K") AS king_items
